...
class TechnicianBreakDownRequestDetailActivity : AppCompatActivity() {
    ...
    private fun breakdownRequestObserver(): Observer<BreakdownRequestResponse> {
        return object : Observer<BreakdownRequestResponse> {
            ...
            override fun onNext(breakdownRequestResponse: BreakdownRequestResponse) {
                ...
                requestId.text=String.format("Request ID: %s",breakdownRequestResponse.breakdownRequest!!.id)
                machineno.text=String.format("Machine NO: %s",breakdownRequestResponse.breakdownRequest!!.machineNumber)
                comment.text = breakdownRequestResponse.breakdownRequest!!.requestDescription
                commentdescription.text=breakdownRequestResponse.breakdownRequest!!.requestDescription
                creat_date.text=String.format("%s %s", breakdownRequestResponse.breakdownRequest!!.requestDate,breakdownRequestResponse.breakdownRequest!!.requestTime)
                suggestedpart.text=String.format("Suggested Part - %s", breakdownRequestResponse.breakdownRequest!!.suggestedPart)
            ...
            override fun onError(e: Throwable) {
                myLoader.dismiss()
                Toast.makeText(this@TechnicianBreakDownRequestDetailActivity,""+e, Toast.LENGTH_SHORT).show()
            }
    ...
    private fun commmentObserver(): Observer<MainResponseModel> {
        return object : Observer<MainResponseModel> {
            ...
            override fun onNext(t: MainResponseModel) {
                ...
                Toast.makeText(this@TechnicianBreakDownRequestDetailActivity,"Comment is successfully",Toast.LENGTH_SHORT).show()
            }
