...
class TechnicianPreventiveRequestDetailActivity : AppCompatActivity() {
    ...
    private fun commmentObserver(): Observer<MainResponseModel> {
        return object : Observer<MainResponseModel> {
            ...
            override fun onNext(t: MainResponseModel) {
                ...
                Toast.makeText(this@TechnicianPreventiveRequestDetailActivity,"Comment is successfully",Toast.LENGTH_SHORT).show()
            }
    ...
    private fun breakdownRequestObserver(): Observer<BreakdownRequestResponse> {
        return object : Observer<BreakdownRequestResponse> {
            ...
            override fun onNext(breakdownRequestResponse: BreakdownRequestResponse) {
                ...
                requestId.text=String.format("Request ID: %s",breakdownRequestResponse.preventiveRequest!!.id)
                machineno.text=String.format("Machine NO: %s",breakdownRequestResponse.preventiveRequest!!.machineNumber)
                comment.text = breakdownRequestResponse.preventiveRequest!!.requestDescription
                commentdescription.text=breakdownRequestResponse.preventiveRequest!!.requestDescription
                creat_date.text=String.format("%s %s", breakdownRequestResponse.preventiveRequest!!.requestDate,breakdownRequestResponse.breakdownRequest!!.requestTime)
                suggestedpart.text=String.format("Suggested Part - %s", breakdownRequestResponse.preventiveRequest!!.suggestedPart)
            ...
            override fun onError(e: Throwable) {
                myLoader.dismiss()
                Toast.makeText(this@TechnicianPreventiveRequestDetailActivity,""+e, Toast.LENGTH_SHORT).show()
            }
