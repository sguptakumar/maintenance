...
import com.appgenesis.com.maintenanceapp.operation_manager.activity.*
import com.appgenesis.com.maintenanceapp.operation_manager.model.DashboardModel

class OperationManagerDashboardAdapter(val mContext: Context?, private val licenceList: ArrayList<DashboardModel>) : RecyclerView.Adapter<OperationManagerDashboardAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        if (licenceList.size != 0) {
            ...
            holder.mainbg.setOnClickListener {
                if (position == 0){
                    val breakDown = Intent(mContext,BreakDownActivity::class.java)
                    breakDown.putExtra("type","breakdown")
                    mContext!!.startActivity(breakDown)
                }
                if (position == 1){
                    val breakDown = Intent(mContext,MaintenanceRequestActivityOperationManager::class.java)
                    breakDown.putExtra("type","mr")
                    mContext!!.startActivity(breakDown)
                }
                if (position == 2){
                    val breakDown = Intent(mContext,OperationManagerPreventiveRequestActivity::class.java)
                    breakDown.putExtra("type","preventive")
                    mContext!!.startActivity(breakDown)
                }
                if (position == 3){
                    val breakDown = Intent(mContext,CompletedRequestActivityOperationManager::class.java)
                    mContext!!.startActivity(breakDown)
                }
                if (position == 4){
                    val breakDown = Intent(mContext,ClosedRequestActivityOperationManager::class.java)
                    mContext!!.startActivity(breakDown)
                }
            }
        }
