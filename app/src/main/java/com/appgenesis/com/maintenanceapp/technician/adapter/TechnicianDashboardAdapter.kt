...
import com.appgenesis.com.maintenanceapp.technician.activity.TechnicianBreakDownRequestActivity
import com.appgenesis.com.maintenanceapp.technician.activity.TechnicianCompletedRequestActivity
import com.appgenesis.com.maintenanceapp.technician.activity.TechnicianMaintenanceRequestActivity
import com.appgenesis.com.maintenanceapp.technician.activity.TechnicianPreventivemaintenanceActivity

class TechnicianDashboardAdapter(val mContext: Context?, private val licenceList: ArrayList<DashboardModel>) : RecyclerView.Adapter<TechnicianDashboardAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        if (licenceList.size != 0) {
            ...
            holder.mainbg.setOnClickListener {
                if (position == 0) {
                    val breakDown = Intent(mContext, TechnicianBreakDownRequestActivity::class.java)
                    breakDown.putExtra("type","breakdown")
                    mContext!!.startActivity(breakDown)
                }
               if (position == 1){
                    val breakDown = Intent(mContext, TechnicianMaintenanceRequestActivity::class.java)
                   breakDown.putExtra("type","mr")
                    mContext!!.startActivity(breakDown)
                }
                if (position == 2){
                    val breakDown = Intent(mContext, TechnicianPreventivemaintenanceActivity::class.java)
                    breakDown.putExtra("type","preventive")
                    mContext!!.startActivity(breakDown)
                }
                if (position == 3){
                    val breakDown = Intent(mContext, TechnicianCompletedRequestActivity::class.java)
                    mContext!!.startActivity(breakDown)
                }
               /* if (position == 4){
