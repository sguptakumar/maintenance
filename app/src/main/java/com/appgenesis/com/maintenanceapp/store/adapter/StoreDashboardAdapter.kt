...
import com.appgenesis.com.maintenanceapp.store.activity.StoreListofPart
import com.appgenesis.com.maintenanceapp.store.activity.StorePreventivemaintenanceActivity
import com.appgenesis.com.maintenanceapp.technician.activity.TechnicianPreventivemaintenanceActivity

class StoreDashboardAdapter(val mContext: Context?, private val licenceList: ArrayList<DashboardModel>) : RecyclerView.Adapter<StoreDashboardAdapter.MyViewHolder>() {
        ...
        if (licenceList.size != 0) {
           ...
           /* if (licenceList[position].name.equals("Preventive Maintenance", true)) {
            ...
            holder.mainbg.setOnClickListener {
                if (position == 0) {
                    val breakDown = Intent(mContext, StoreListofPart::class.java)
                    mContext!!.startActivity(breakDown)
                }
               if (position == 1){
                    val breakDown = Intent(mContext, StorePreventivemaintenanceActivity::class.java)
                    mContext!!.startActivity(breakDown)
                }
