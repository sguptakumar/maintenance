...
import com.appgenesis.com.maintenanceapp.management.adapter.ManagementMangerCommentAdapter
import com.appgenesis.com.maintenanceapp.management.adapter.ManagementOMGalleryAdapter
...
class ManagementPreventiveRequestDetailActivity : AppCompatActivity() {
    internal var pdfUri: Uri? = null
    var path:String?=null
    protected lateinit var commentAdapter: ManagementMangerCommentAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_management_preventive_request_detail)
        ...
        myLoader.show()
        commentAdapter= ManagementMangerCommentAdapter(this, commentList)
        ...
        breakdown_detail_galleryrecyclerview.adapter = ManagementOMGalleryAdapter(this, gallryList)
        ...
        ApiClient.getClient().create(NetworkApi::class.java)
            .getPreventiveRequestDetails(Constant.getHeaderMap(), requestId)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
    ...
    private fun commmentObserver(): Observer<MainResponseModel> {
        return object : Observer<MainResponseModel> {
            ...
            override fun onNext(t: MainResponseModel) {
                ...
                Toast.makeText(this@ManagementPreventiveRequestDetailActivity,"Comment is successfully",Toast.LENGTH_SHORT).show()
    ...
    private fun breakdownRequestObserver(): Observer<BreakdownRequestResponse> {
        return object : Observer<BreakdownRequestResponse> {
            ...
            override fun onNext(breakdownRequestResponse: BreakdownRequestResponse) {
                ...
                breakdown_detail_requestId.text = String.format("Request ID: %s",breakdownRequestResponse.preventiveRequest!!.id)
                breakdown_detail_machineno.text = String.format("Machine NO: %s",breakdownRequestResponse.preventiveRequest!!.machineNumber)
                breakdown_detail_comment.text = breakdownRequestResponse.preventiveRequest!!.requestDescription
                breakdown_detail_commentdescription.text = breakdownRequestResponse.preventiveRequest!!.requestDescription
                breakdown_detail_creat_date.text =String.format("%s %s", breakdownRequestResponse.preventiveRequest!!.requestDate,breakdownRequestResponse.preventiveRequest!!.requestTime)
                breakdown_detail_suggestedpart.text =String.format("Suggested Part - %s", breakdownRequestResponse.preventiveRequest!!.suggestedPart)
                var comment=breakdownRequestResponse.preventiveRequest.comments
